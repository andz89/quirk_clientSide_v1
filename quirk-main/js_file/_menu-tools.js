...
"_controlsVisibility","lockMovementX","lockMovementY","lockScalingX","lockScalingY","selectable","textAlign","fontFamily", "id", "name",'clip_image_src_org',"orig_url"])
...
let size = {w:this.width, h:this.height};
    ...
    let image_object = this.canvas.getActiveObject()
      ...
      this.canvas.exist_bottom = b - a
        ...
        img.original_image_width = image_display.getScaledWidth();
          ...
          all_objects.forEach((obj)=>{this.lock_image(obj, false)})
      ...
      document.querySelector('.save_cancel_crop #cancel').onclick = ()=>{
        ...
        const cancel_cropping =()=>{
          ...
          image_object.visible = true//show hide object
...
this.canvas.getObjects().forEach((obj)=>{this.lock_image(obj, false)}) //unclock all objects
  }
...
const cancel_cropping = () =>{
  ...
  all_objects.forEach((obj)=>{this.lock_image(obj, false) }) //unclock all objects
  image_object.top = this.canvas.current_top;
  image_object.left = this.canvas.current_left
}
      }
     ...
     let cropper_box = new fabric.Circle({
      ...
      this.canvas.add(cropper_box)
        ...
        document.querySelector('.save_cancel_clip #save').onclick = ()=>{
          ...
          clipPath: new fabric.Circle({ radius: cropper_box.getScaledWidth()/2 , top: a, left: b,   absolutePositioned: true})
          });
